...
const STATUSES = [
  { key: "active", label: "Active", color: "bg-blue-50 text-blue-700 border-blue-200" },
  { key: "under_contract", label: "Under Contract", color: "bg-amber-50 text-amber-700 border-amber-200" },
  { key: "closed", label: "Closed", color: "bg-emerald-50 text-emerald-700 border-emerald-200" },
  { key: "archived", label: "Archived", color: "bg-gray-50 text-gray-500 border-gray-200" },
];
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6]">
      {/* Nav */}
      <nav className="bg-white border-b border-[#F0EDE6] sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto p-3 sm:p-4 lg:p-8">
        {/* Header */}
        <div className="flex items-center justify-between mb-4 lg:mb-6 animate-fade-in">
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            <div>
              ...
              <p className="text-xs lg:text-sm text-[#8A8272]">{pipelineStats.total} total deals · ${Number(totalValue).toLocaleString()} total volume</p>
            </div>
          </div>
          <button onClick={() => setShowManual(!showManual)}
            className="btn btn-primary text-xs lg:text-sm whitespace-nowrap">
            + Add Deal
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-4 mb-4 lg:mb-6">
          {STATUSES.map((s, i) => (
            <div key={s.key} className={`stat-card animate-fade-in-${i + 1} border-l-[3px] ${s.key === "active" ? "border-l-blue-500" : s.key === "under_contract" ? "border-l-amber-500" : s.key === "closed" ? "border-l-emerald-500" : "border-l-gray-400"}`}>
              <p className="label">{STATUS_ICONS[s.key]} {s.label}</p>
              <p className="value">{pipelineStats[s.key] || 0}</p>
            </div>
          ))}
        </div>
        ...
        {/* Manual Deal Form */}
        {showManual && (
          <div className="bg-white rounded-xl p-4 lg:p-6 border border-[#F0EDE6] mb-4 lg:mb-6 animate-fade-in">
            <h3 className="text-sm font-semibold text-[#1C1810] mb-3">Add Deal Manually</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
              <input id="manual-address" className="input-field" placeholder="Property address" />
              <input id="manual-price" className="input-field" placeholder="Purchase price" type="number" />
              <select id="manual-status" className="input-field bg-white">
            ...
            <div key={column.key} className="bg-[#FAF9F6] rounded-xl p-3 lg:p-4 border border-[#F0EDE6] min-h-[200px]">
              ...
              <div className="space-y-2">
                {(dealsByStatus[column.key] || []).slice(0, 8).map((deal) => (
                  <div key={deal.id}
                    className="bg-white rounded-xl p-3 border border-[#F0EDE6] hover:shadow-sm transition-shadow cursor-pointer text-xs group">
                ...
                {(!dealsByStatus[column.key] || dealsByStatus[column.key].length === 0) && (
                  <p className="text-[10px] text-[#A39C8E] text-center py-6">No deals yet</p>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-6 mb-4 lg:mb-8">
          <div className="bg-white rounded-xl p-4 lg:p-6 border border-[#F0EDE6]">
            <h2 className="text-xs font-semibold text-[#453D30] mb-3 lg:mb-4 uppercase tracking-wider">Extractions Over Time</h2>
            ...
            ) : <p className="text-[#A39C8E] text-xs text-center py-8 lg:py-16">No data yet</p>}
          </div>
          <div className="bg-white rounded-xl p-4 lg:p-6 border border-[#F0EDE6]">
            <h2 className="text-xs font-semibold text-[#453D30] mb-3 lg:mb-4 uppercase tracking-wider">Contingency Types</h2>
        ...
        {/* Recent Activity */}
        <div className="bg-white rounded-xl border border-[#F0EDE6] overflow-hidden">
          <div className="px-4 lg:px-6 py-3 lg:py-4 border-b border-[#F0EDE6] flex items-center justify-between">
            <div className="flex items-center gap-1.5 lg:gap-2">
              <span className="text-xs lg:text-sm">📋</span>
              <h2 className="text-xs font-semibold text-[#453D30] uppercase tracking-wider">Recent Activity</h2>
            </div>
